...
@Component({
  selector: 'app-don-vi-tinh-chi-tiet',
  templateUrl: './don-vi-tinh-chi-tiet.component.html',
  styleUrls: ['./don-vi-tinh-chi-tiet.component.scss']
})
export class DonViTinhChiTietComponent implements OnInit {
  ...
  constructor(private dialog: MatDialog, private baseService: BaseService, private snackBar: MatSnackBar, private errorService: ErrorService,
    private donViTinhService: DonViTinhService, private authService: AuthService,
    private cdRef: ChangeDetectorRef) {
    this.baseService.controllerName = this.donViTinhService.controllerName;
  }
  ...
  capNhat() {
    ...
    if (this.authService.hasPermission(this.documentType, SecurityOperation.Update)) {
      let dialogRef = this.dialog.open(ConfirmComponent, {
        data: {
          message: "Bạn chắc chắn muốn cập nhật đơn vị tính này?"
        }
      });
      dialogRef.afterClosed().subscribe(result => {
        if (result == true) {
          this.showPopupLoadingData();
          this.baseService.update(this.donViTinhVo).subscribe(() => {
            this.dataChange.emit(this.donViTinhVo);
  ...
  luu() {
    ...
    if (this.authService.hasPermission(this.documentType, SecurityOperation.Add)) {
      let dialogRef = this.dialog.open(ConfirmComponent, {
        data: {
          message: "Bạn chắc chắn muốn lưu đơn vị tính này?"
        }
      });
      dialogRef.afterClosed().subscribe(result => {
        if (result == true) {
          this.showPopupLoadingData();
          this.baseService.create(this.donViTinhVo).subscribe((result) => {
            this.donViTinhVo = {};
  ...
  hieuLucChange(event:any){
    if(event != null){
      if(event == 1){
        this.donViTinhVo.HieuLuc = true;
      }
      if(event == 2)
      {
        this.donViTinhVo.HieuLuc = false;
      }
    }
    else{
      this.donViTinhVo.HieuLuc = null;
    }
