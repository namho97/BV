...
@Component({
  selector: 'app-vi-tri-de-duoc-pham-vat-tu-chi-tiet',
  templateUrl: './vi-tri-de-duoc-pham-vat-tu-chi-tiet.component.html',
  styleUrls: ['./vi-tri-de-duoc-pham-vat-tu-chi-tiet.component.scss']
})
export class ViTriDeDuocPhamVatTuChiTietComponent implements OnInit {

  id :number = null;
  viTriDPVTVo: any = {};
  validationErrors: any = null;
  documentType: DocumentType = DocumentType.QuanTriNhomKhoViTriDeDuocPhamVatTu;
  ...
  constructor(private dialog: MatDialog, private baseService: BaseService, private snackBar: MatSnackBar, private errorService: ErrorService,
    private viTriDuocPhamVatTuService: ViTriDuocPhamVatTuService, private authService: AuthService,
    private cdRef: ChangeDetectorRef) {
    this.baseService.controllerName = this.viTriDuocPhamVatTuService.controllerName;
  }
  ...
  capNhat() {
    ...
    if (this.authService.hasPermission(this.documentType, SecurityOperation.Update)) {
      let dialogRef = this.dialog.open(ConfirmComponent, {
        data: {
          message: "Bạn chắc chắn muốn cập nhật vị trí dp/vt này?"
        }
      });
      dialogRef.afterClosed().subscribe(result => {
        if (result == true) {
          this.showPopupLoadingData();
          this.baseService.update(this.viTriDPVTVo).subscribe(() => {
            this.dataChange.emit(this.viTriDPVTVo);
  ...
  luu() {
    ...
    if (this.authService.hasPermission(this.documentType, SecurityOperation.Add)) {
      let dialogRef = this.dialog.open(ConfirmComponent, {
        data: {
          message: "Bạn chắc chắn muốn lưu vị trí dp/vt này?"
        }
      });
      dialogRef.afterClosed().subscribe(result => {
        if (result == true) {
          this.showPopupLoadingData();
          this.baseService.create(this.viTriDPVTVo).subscribe((result) => {
            this.viTriDPVTVo = {};
