...
@Component({
  selector: 'app-dan-toc-chi-tiet',
  templateUrl: './dan-toc-chi-tiet.component.html',
  styleUrls: ['./dan-toc-chi-tiet.component.scss']
})
export class DanTocChiTietComponent implements OnInit {

  id :number = null;
  danTocVo: any = {};
  validationErrors: any = null;
  documentType: DocumentType = DocumentType.QuanTriNhomHanhChinhDanToc;
  ...
  constructor(private dialog: MatDialog, private baseService: BaseService, private snackBar: MatSnackBar, private errorService: ErrorService,
    private danTocService: DanTocService, private authService: AuthService,
    private cdRef: ChangeDetectorRef) {
    this.baseService.controllerName = this.danTocService.controllerName;
  }
  ...
  capNhat() {
    ...
    if (this.authService.hasPermission(this.documentType, SecurityOperation.Update)) {
      let dialogRef = this.dialog.open(ConfirmComponent, {
        data: {
          message: "Bạn chắc chắn muốn cập nhật dân tộc này?"
        }
      });
      dialogRef.afterClosed().subscribe(result => {
        if (result == true) {
          this.showPopupLoadingData();
          this.baseService.update(this.danTocVo).subscribe(() => {
            this.dataChange.emit(this.danTocVo);
  ...
  luu() {
    ...
    if (this.authService.hasPermission(this.documentType, SecurityOperation.Add)) {
      let dialogRef = this.dialog.open(ConfirmComponent, {
        data: {
          message: "Bạn chắc chắn muốn lưu dân tộc này?"
        }
      });
      dialogRef.afterClosed().subscribe(result => {
        if (result == true) {
          this.showPopupLoadingData();
          this.baseService.create(this.danTocVo).subscribe((result) => {
            this.danTocVo = {};
  ...
  hieuLucChange(event:any){
    if(event != null){
      if(event == 1){
        this.danTocVo.HieuLuc = true;
      }
      if(event == 2)
      {
        this.danTocVo.HieuLuc = false;
      }
    }
    else{
      this.danTocVo.HieuLuc = null;
    }
