...
@Component({
  selector: 'app-dich-vu-kham',
  templateUrl: './dich-vu-kham.component.html',
  styleUrls: ['./dich-vu-kham.component.scss']
})
export class DichVuKhamComponent implements OnInit {

  id: number = null;
  dataChon: any;
  now:Date=new Date();
  @ViewChild("tableDichVuKham", { static: true }) table:TableComponent;
  ...
  dichVuKhamQueryInfo:any={};
  dichVuKhamSearching:boolean=false;
  columnsDichVuKham: TableColumn<any>[] = [];
  columnsChildDichVuKham: TableColumn<any>[] = [];
  groupByColumnsDichVuKham: string[] = [];
  sortByColumnDichVuKham: SortDescriptor = { field: 'Ma', dir: 'desc' };
  documentTypeDichVuKham: DocumentType;

  initDichVuKham() {
    this.documentTypeDichVuKham = DocumentType.QuanTriNhomPhongKhamDichVuKham;
    this.columnsDichVuKham = [
      ...
      { label: 'Mô tả', property: 'MoTa', type: 'text', width: 100, visible: true },
    ...
    this.columnsChildDichVuKham = [
      { label: 'Giá', property: 'Gia', type: 'text', minWidth: 70, visible: true, sortable: true },
      { label: 'Từ ngày', property: 'TuNgay', type: 'text', width: 100, visible: true },
      { label: 'Đến ngày', property: 'DenNgay', type: 'text', width: 200, visible: true },
    ];
  }
  dangTimKiem:boolean=false;
  timKiem() {
    this.table.additionalSearchObject = this.dichVuKhamQueryInfo;
    this.table.search();
    this.dangTimKiem=true;
    this.dichVuKhamSearching=true;
  }
  huyTimKiem(){
    this.dichVuKhamQueryInfo={};
    this.timKiem();
    this.dangTimKiem=false;
    this.dichVuKhamSearching=false;
  }
  onKey(event: any) {
    if (event.key == 'Enter') {
        this.timKiem();
  ...
  updateDichVuKham(id: number, hieuLuc: boolean) {
    hieuLuc 
    id
    ...
    if (this.authService.hasPermission(this.documentTypeDichVuKham, SecurityOperation.Update)) {

      this.dialog.open(ConfirmComponent, {
        disableClose: false,
        width: '400px',
        data: { message: CommonService.format(SystemMessage.MessLockTemplate, [comfirm, "dịch vụ khám"]) }
      }).afterClosed().subscribe(result => {/* result is a string:Yes,No,No answer*/
        if (result == true) {
          self.apiService.post("QuanTriNhomPhongKhamDichVuKham/KichHoatHieuLuc?id=" + id).subscribe(
            () => {
              self.table.search();
              if (hieuLuc)
