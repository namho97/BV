...
@Component({
  selector: 'app-quan-he-than-nhan-chi-tiet',
  templateUrl: './quan-he-than-nhan-chi-tiet.component.html',
  styleUrls: ['./quan-he-than-nhan-chi-tiet.component.scss']
})
export class QuanHeThanNhanChiTietComponent implements OnInit {

  id :number = null;
  quanHeThanNhanVo: any = {};
  validationErrors: any = null;
  documentType: DocumentType = DocumentType.QuanTriNhomNguoiBenhQuanHeThanNhan;
  ...
  constructor(private dialog: MatDialog, private baseService: BaseService, private snackBar: MatSnackBar, private errorService: ErrorService,
    private quanHeThanNhanService: QuanHeThanNhanService, private authService: AuthService,
    private cdRef: ChangeDetectorRef) {
    this.baseService.controllerName = this.quanHeThanNhanService.controllerName;
  }
  ...
  capNhat() {
    ...
    if (this.authService.hasPermission(this.documentType, SecurityOperation.Update)) {
      let dialogRef = this.dialog.open(ConfirmComponent, {
        data: {
          message: "Bạn chắc chắn muốn cập nhật quan hệ thân nhân này?"
        }
      });
      dialogRef.afterClosed().subscribe(result => {
        if (result == true) {
          this.showPopupLoadingData();
          this.baseService.update(this.quanHeThanNhanVo).subscribe(() => {
            this.dataChange.emit(this.quanHeThanNhanVo);
  ...
  luu() {
    ...
    if (this.authService.hasPermission(this.documentType, SecurityOperation.Add)) {
      let dialogRef = this.dialog.open(ConfirmComponent, {
        data: {
          message: "Bạn chắc chắn muốn lưu quan hệ thân nhân này?"
        }
      });
      dialogRef.afterClosed().subscribe(result => {
        if (result == true) {
          this.showPopupLoadingData();
          this.baseService.create(this.quanHeThanNhanVo).subscribe((result) => {
            this.quanHeThanNhanVo = {};
  ...
  hieuLucChange(event:any){
    if(event != null){
      if(event == 1){
        this.quanHeThanNhanVo.HieuLuc = true;
      }
      if(event == 2)
      {
        this.quanHeThanNhanVo.HieuLuc = false;
      }
    }
    else{
      this.quanHeThanNhanVo.HieuLuc = null;
    }
