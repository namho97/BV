...
@Component({
  selector: 'app-van-ban-chuyen-mon',
  templateUrl: './van-ban-chuyen-mon.component.html',
  styleUrls: ['./van-ban-chuyen-mon.component.scss']
})
export class VanBanChuyenMonComponent implements OnInit {

  id: number = null;
  dataChon: any;
  now:Date=new Date();
  @ViewChild("tableVanBangChuyenMon", { static: true }) table:TableComponent;
  ...
  vanBangChuyenMonQueryInfo:any={};
  vanBangChuyenMonSearching:boolean=false;
  columnsVanBangChuyenMon: TableColumn<any>[] = [];
  groupByColumnsVanBangChuyenMon: string[] = [];
  sortByColumnVanBangChuyenMon: SortDescriptor = { field: 'Ma', dir: 'desc' };
  documentTypeVanBangChuyenMon: DocumentType;

  initVanBangChuyenMon() {
    this.documentTypeVanBangChuyenMon = DocumentType.QuanTriNhomHanhChinhVanBangChuyenMon;
    this.columnsVanBangChuyenMon = [
      ...
      { label: 'Tên viết tắt', property: 'TenVietTat', type: 'text', width: 150, visible: true },
  ...
  timKiem() {
    this.table.additionalSearchObject = this.vanBangChuyenMonQueryInfo;
    this.table.search();
    this.dangTimKiem=true;
    this.vanBangChuyenMonSearching=true;
  }
  huyTimKiem(){
    this.vanBangChuyenMonQueryInfo={};
    this.timKiem();
    this.dangTimKiem=false;
    this.vanBangChuyenMonSearching=false;
  }
  //#region actions
  ...
  updateVanBangChuyenMon(id: number, hieuLuc: boolean) {
    hieuLuc 
    id
    ...
    if (this.authService.hasPermission(this.documentTypeVanBangChuyenMon, SecurityOperation.Update)) {

      this.dialog.open(ConfirmComponent, {
        disableClose: false,
        width: '400px',
        data: { message: CommonService.format(SystemMessage.MessLockTemplate, [comfirm, "văn bằng chuyên môn"]) }
      }).afterClosed().subscribe(result => {/* result is a string:Yes,No,No answer*/
        if (result == true) {
          self.apiService.post("QuanTriNhomHanhChinhVanBangChuyenMon/KichHoatHieuLuc?id=" + id).subscribe(
            () => {
              self.table.search();
              if (hieuLuc)
