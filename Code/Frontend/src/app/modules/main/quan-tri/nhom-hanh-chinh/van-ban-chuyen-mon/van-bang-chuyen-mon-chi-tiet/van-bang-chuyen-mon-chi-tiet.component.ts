...
@Component({
  selector: 'app-van-bang-chuyen-mon-chi-tiet',
  templateUrl: './van-bang-chuyen-mon-chi-tiet.component.html',
  styleUrls: ['./van-bang-chuyen-mon-chi-tiet.component.scss']
})
export class VanBangChuyenMonChiTietComponent implements OnInit {

  id:number = null;
  vanBangChuyenMonVo: any = {};
  validationErrors: any = null;
  documentType: DocumentType = DocumentType.QuanTriNhomHanhChinhVanBangChuyenMon;
  ...
  constructor(private dialog: MatDialog, private baseService: BaseService, private snackBar: MatSnackBar, private errorService: ErrorService,
    private vanBangChuyenMonService: VanBangChuyenMonService, private authService: AuthService,
    private cdRef: ChangeDetectorRef) {
    this.baseService.controllerName = this.vanBangChuyenMonService.controllerName;
  }
  ...
  getById(id) {
    if (id != null && id > 0) {
      this.baseService.getById(id).subscribe((result) => {
        this.vanBangChuyenMonVo = result;
      });
    }
  }
  quayLai() {
  ...
  capNhat() {
    ...
    if (this.authService.hasPermission(this.documentType, SecurityOperation.Update)) {
      let dialogRef = this.dialog.open(ConfirmComponent, {
        data: {
          message: "Bạn chắc chắn muốn cập nhật văn bằng chuyên môn này?"
        }
      });
      dialogRef.afterClosed().subscribe(result => {
        if (result == true) {
          this.showPopupLoadingData();
          this.baseService.update(this.vanBangChuyenMonVo).subscribe(() => {
            this.dataChange.emit(this.vanBangChuyenMonVo);
  ...
  luu() {
    ...
    if (this.authService.hasPermission(this.documentType, SecurityOperation.Add)) {
      let dialogRef = this.dialog.open(ConfirmComponent, {
        data: {
          message: "Bạn chắc chắn muốn lưu văn bằng chuyên môn này?"
        }
      });
      dialogRef.afterClosed().subscribe(result => {
        if (result == true) {
          this.showPopupLoadingData();
          this.baseService.create(this.vanBangChuyenMonVo).subscribe((result) => {
            this.vanBangChuyenMonVo = {};
  ...
  hieuLucChange(event:any){
    if(event != null){
      if(event == 1){
        this.vanBangChuyenMonVo.HieuLuc = true;
      }
      if(event == 2)
      {
        this.vanBangChuyenMonVo.HieuLuc = false;
      }
    }
    else{
      this.vanBangChuyenMonVo.HieuLuc = null;
    }
