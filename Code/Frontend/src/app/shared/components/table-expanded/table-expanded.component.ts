...
import { Group, SortDescriptor, TableColumn, TableQueryInfo } from './table-expanded.model';
declare let $: any;

@Component({
  selector: 'app-table-expanded',
  templateUrl: './table-expanded.component.html',
  styleUrls: ['./table-expanded.component.scss'],
  animations: [
...
export class TableExpandedComponent implements OnInit,AfterViewInit {
  ...
  @Input() heightToolbar: number=211;
  ...
  @Input() tableDetailTemplate: any = null;
  ...
  ngOnInit() {
    this.groupByColumns=null;//Chưa làm được group trong table này
    this.skip = 0;
 ...
 getTotalWidth(){
  ...
  setDataSource(dataSource:any) {
    ...
    if (dataSource == null) {      
      ...
      setTimeout(function(){
        ...
        console.log(self._tableDataSource);
      });
    }   
    if(this.columns.filter(o=>o.type=="expanded").length<=0)
    {
      this.columns.unshift({ label: '', property: 'Expanded', type: 'expanded',width:30, visible: true ,hideFilter:true });
    } 
  }
  ...
  search(){
    this.onFilterChange(this.searchCtrl!=null?this.searchCtrl.value:null);
