...
@Component({
  selector: 'app-nha-san-xuat-chi-tiet',
  templateUrl: './nha-san-xuat-chi-tiet.component.html',
  styleUrls: ['./nha-san-xuat-chi-tiet.component.scss']
})
export class NhaSanXuatChiTietComponent implements OnInit {

  id:number = null;
  nhaSanXuatVo: any = {};
  validationErrors: any = null;
  documentType: DocumentType = DocumentType.QuanTriNhomDuocPhamNhaSanXuat;
  ...
  constructor(private dialog: MatDialog, private baseService: BaseService, private snackBar: MatSnackBar, private errorService: ErrorService,
    private nhaSanXuatService: NhaSanXuatService, private authService: AuthService,
    private cdRef: ChangeDetectorRef) {
    this.baseService.controllerName = this.nhaSanXuatService.controllerName;
  }
  ...
  capNhat() {
    ...
    if (this.authService.hasPermission(this.documentType, SecurityOperation.Update)) {
      let dialogRef = this.dialog.open(ConfirmComponent, {
        data: {
          message: "Bạn chắc chắn muốn cập nhật nhà sản xuất này?"
        }
      });
      dialogRef.afterClosed().subscribe(result => {
        if (result == true) {
          this.showPopupLoadingData();
          this.baseService.update(this.nhaSanXuatVo).subscribe(() => {
            this.dataChange.emit(this.nhaSanXuatVo);
  ...
  luu() {
    ...
    if (this.authService.hasPermission(this.documentType, SecurityOperation.Add)) {
      let dialogRef = this.dialog.open(ConfirmComponent, {
        data: {
          message: "Bạn chắc chắn muốn lưu nhà sản xuất này?"
        }
      });
      dialogRef.afterClosed().subscribe(result => {
        if (result == true) {
          this.showPopupLoadingData();
          this.baseService.create(this.nhaSanXuatVo).subscribe((result) => {
            this.nhaSanXuatVo = {};
  ...
  hieuLucChange(event:any){
    if(event != null){
      if(event == 1){
        this.nhaSanXuatVo.HieuLuc = true;
      }
      if(event == 2)
      {
        this.nhaSanXuatVo.HieuLuc = false;
      }
    }
    else{
      this.nhaSanXuatVo.HieuLuc = null;
    }
