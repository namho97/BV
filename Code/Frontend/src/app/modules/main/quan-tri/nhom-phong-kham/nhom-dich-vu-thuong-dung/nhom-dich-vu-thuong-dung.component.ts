...
export class NhomDichVuThuongDungComponent implements OnInit {
  ...
  dataChon: any;
  now:Date=new Date();
  @ViewChild("tableNhomDichVuThuongDung", { static: true }) table:TableComponent;
  ...
  nhomDichVuThuongDungQueryInfo:any={};
  nhomDichVuThuongDungSearching:boolean=false;
  columnsNhomDichVuThuongDung: TableColumn<any>[] = [];
  groupByColumnsNhomDichVuThuongDung: string[] = [];
  sortByColumnNhomDichVuThuongDung: SortDescriptor = { field: 'TenNhom', dir: 'desc' };
  documentTypeNhomDichVuThuongDung: DocumentType;

  initNhomDichVuThuongDung() {
    this.documentTypeNhomDichVuThuongDung = DocumentType.QuanTriNhomPhongKhamNhomDichVuThuongDung;
    this.columnsNhomDichVuThuongDung = [
      { label: 'TÊN NHÓM', property: 'TenNhom', type: 'text', minWidth: 300, visible: true,isLink:true,sticky:true },
  ...
  timKiem() {
    this.table.additionalSearchObject = this.nhomDichVuThuongDungQueryInfo;
    this.table.search();
    this.dangTimKiem=true;
    this.nhomDichVuThuongDungSearching=true;
  }
  huyTimKiem(){
    this.nhomDichVuThuongDungQueryInfo={};
    this.timKiem();
    this.dangTimKiem=false;
    this.nhomDichVuThuongDungSearching=false;
  }
  onKey(event: any) {
    if (event.key == 'Enter') {
        this.timKiem();
  ...
  updateNhomDichVuThuongDung(id: number, hieuLuc: boolean) {
    hieuLuc 
    id
    ...
    if (this.authService.hasPermission(this.documentTypeNhomDichVuThuongDung, SecurityOperation.Update)) {

      this.dialog.open(ConfirmComponent, {
        disableClose: false,
        width: '400px',
        data: { message: CommonService.format(SystemMessage.MessLockTemplate, [comfirm, "nhóm dịch vụ thường dùng"]) }
      }).afterClosed().subscribe(result => {/* result is a string:Yes,No,No answer*/
        if (result == true) {
          self.apiService.post("QuanTriNhomPhongKhamNhomDichVuThuongDung/KichHoatHieuLuc?id=" + id).subscribe(
            () => {
              self.table.search();
              if (hieuLuc)
