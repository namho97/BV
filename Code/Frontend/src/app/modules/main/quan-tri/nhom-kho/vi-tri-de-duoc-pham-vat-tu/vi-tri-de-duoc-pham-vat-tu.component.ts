...
@Component({
  selector: 'app-vi-tri-de-duoc-pham-vat-tu',
  templateUrl: './vi-tri-de-duoc-pham-vat-tu.component.html',
  styleUrls: ['./vi-tri-de-duoc-pham-vat-tu.component.scss']
})
export class ViTriDeDuocPhamVatTuComponent implements OnInit {

  id: number = null;
  dataChon: any;
  now:Date=new Date();
  @ViewChild("tableViTriDPVT", { static: true }) table:TableComponent;
  ...
  viTriDPVTQueryInfo:any={};
  viTriDPVTSearching:boolean=false;
  columnsViTriDPVT: TableColumn<any>[] = [];
  groupByColumnsViTriDPVT: string[] = [];
  sortByColumnViTriDPVT: SortDescriptor = { field: 'Ten', dir: 'desc' };
  documentTypeViTriDPVT: DocumentType;

  initViTriDPVT() {
    this.documentTypeViTriDPVT = DocumentType.QuanTriNhomKhoViTriDeDuocPhamVatTu;
    this.columnsViTriDPVT = [
      { label: 'Tên', property: 'Ten', type: 'text', width: 78, visible: true,isLink:true,sticky:true },
      { label: 'Mô tả', property: 'MoTa', type: 'text', width: 300, visible: true},
      { label: 'Kho', property: 'Kho', type: 'text', minWidth: 100, visible: true },
  ...
  timKiem() {
    this.table.additionalSearchObject = this.viTriDPVTQueryInfo;
    this.table.search();
    this.dangTimKiem=true;
    this.viTriDPVTSearching=true;
  }
  huyTimKiem(){
    this.viTriDPVTQueryInfo={};
    this.timKiem();
    this.dangTimKiem=false;
    this.viTriDPVTSearching=false;
  }
  onKey(event: any) {
    if (event.key == 'Enter') {
        this.timKiem();
  ...
  updateViTriDPVT(id: number, hieuLuc: boolean) {
    hieuLuc 
    id
    ...
    if (this.authService.hasPermission(this.documentTypeViTriDPVT, SecurityOperation.Update)) {

      this.dialog.open(ConfirmComponent, {
        disableClose: false,
        width: '400px',
        data: { message: CommonService.format(SystemMessage.MessLockTemplate, [comfirm, "vị trí dp/vt"]) }
      }).afterClosed().subscribe(result => {/* result is a string:Yes,No,No answer*/
        if (result == true) {
          self.apiService.post("QuanTriNhomKhoViTriDeDuocPhamVatTu/KichHoatHieuLuc?id=" + id).subscribe(
            () => {
              self.table.search();
              if (hieuLuc)
