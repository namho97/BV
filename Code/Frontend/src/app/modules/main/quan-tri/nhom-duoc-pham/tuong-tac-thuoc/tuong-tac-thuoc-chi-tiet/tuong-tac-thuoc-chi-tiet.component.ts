...
@Component({
  selector: 'app-tuong-tac-thuoc-chi-tiet',
  templateUrl: './tuong-tac-thuoc-chi-tiet.component.html',
  styleUrls: ['./tuong-tac-thuoc-chi-tiet.component.scss']
})
export class TuongTacThuocChiTietComponent implements OnInit {

  id :number = null;
  tuongTacThuocVo: any = {};
  validationErrors: any = null;
  documentType: DocumentType = DocumentType.QuanTriNhomDuocPhamTuongTacThuoc;
  ...
  constructor(private dialog: MatDialog, private baseService: BaseService, private snackBar: MatSnackBar, private errorService: ErrorService,
    private tuongTacThuocService: TuongTacThuocService, private authService: AuthService,
    private cdRef: ChangeDetectorRef) {
    this.baseService.controllerName = this.tuongTacThuocService.controllerName;
  }
  ...
  capNhat() {
    ...
    if (this.authService.hasPermission(this.documentType, SecurityOperation.Update)) {
      let dialogRef = this.dialog.open(ConfirmComponent, {
        data: {
          message: "Bạn chắc chắn muốn cập nhật tương tác thuốc này?"
        }
      });
      dialogRef.afterClosed().subscribe(result => {
        if (result == true) {
          this.showPopupLoadingData();
          this.baseService.update(this.tuongTacThuocVo).subscribe(() => {
            this.dataChange.emit(this.tuongTacThuocVo);
  ...
  luu() {
    ...
    if (this.authService.hasPermission(this.documentType, SecurityOperation.Add)) {
      let dialogRef = this.dialog.open(ConfirmComponent, {
        data: {
          message: "Bạn chắc chắn muốn lưu tương tác thuốc này?"
        }
      });
      dialogRef.afterClosed().subscribe(result => {
        if (result == true) {
          this.showPopupLoadingData();
          this.baseService.create(this.tuongTacThuocVo).subscribe((result) => {
            this.tuongTacThuocVo = {};
  ...
  hieuLucChange(event:any){
    if(event != null){
      if(event == 1){
        this.tuongTacThuocVo.HieuLuc = true;
      }
      if(event == 2)
      {
        this.tuongTacThuocVo.HieuLuc = false;
      }
    }
    else{
      this.tuongTacThuocVo.HieuLuc = null;
    }
