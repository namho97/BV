...
export class DanTocComponent implements OnInit {
  ...
  dataChon: any;
  now: Date = new Date();
  @ViewChild("tableDanToc", { static: true }) table: TableComponent;
  ...
  danTocQueryInfo: any = {};
  danTocSearching: boolean = false;
  columnsDanToc: TableColumn<any>[] = [];
  groupByColumnsDanToc: string[] = [];
  sortByColumnDanToc: SortDescriptor = { field: 'Ma', dir: 'desc' };
  documentTypeDanToc: DocumentType;
  @ViewChild('groupTemplate', { static: true }) groupTemplate: TemplateRef<any>;

  initDanToc() {
    this.documentTypeDanToc = DocumentType.QuanTriNhomHanhChinhDanToc;
    this.columnsDanToc = [
      { label: 'Mã', property: 'Ma', type: 'text', width: 78, visible: true },
      { label: 'Tên', property: 'Ten', type: 'text', minWidth: 300, visible: true, isLink: true, sticky: true },
      { label: 'Quốc gia', property: 'QuocGia', type: 'text', width: 150, visible: true },
  ...
  timKiem() {
    this.table.additionalSearchObject = this.danTocQueryInfo;
    this.table.search();
    this.dangTimKiem = true;
    this.danTocSearching = true;
  }
  huyTimKiem() {
    this.danTocQueryInfo = {};
    this.timKiem();
    this.dangTimKiem = false;
    this.danTocSearching = false;
  }
  //#region actions
  ...
  updateDanToc(id: number, hieuLuc: boolean) {
    hieuLuc
    id
    ...
    if (this.authService.hasPermission(this.documentTypeDanToc, SecurityOperation.Update)) {

      this.dialog.open(ConfirmComponent, {
        disableClose: false,
        width: '400px',
        data: { message: CommonService.format(SystemMessage.MessLockTemplate, [comfirm, "dân tộc"]) }
      }).afterClosed().subscribe(result => {/* result is a string:Yes,No,No answer*/
        if (result == true) {
          self.apiService.post("QuanTriNhomHanhChinhDanToc/KichHoatHieuLuc?id=" + id).subscribe(
            () => {
              self.table.search();
              if (hieuLuc)
