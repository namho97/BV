...
export class DonViTinhComponent implements OnInit {
  ...
  dataChon: any;
  now:Date=new Date();
  @ViewChild("tableDonVitinh", { static: true }) table:TableComponent;
  ...
  donViTinhQueryInfo:any={};
  donViTinhSearching:boolean=false;
  columnsDonViTinh: TableColumn<any>[] = [];
  groupByColumnsDonViTinh: string[] = [];
  sortByColumnDonViTinh: SortDescriptor = { field: 'Ma', dir: 'desc' };
  documentTypeDonViTinh: DocumentType;

  initDonViTinh() {
    this.documentTypeDonViTinh = DocumentType.QuanTriNhomDuocPhamDonViTinh;
    this.columnsDonViTinh = [
  ...
  timKiem() {
    this.table.additionalSearchObject = this.donViTinhQueryInfo;
    this.table.search();
    this.dangTimKiem=true;
    this.donViTinhSearching=true;
  }
  huyTimKiem(){
    this.donViTinhQueryInfo={};
    this.timKiem();
    this.dangTimKiem=false;
    this.donViTinhSearching=false;
  }
  onKey(event: any) {
    if (event.key == 'Enter') {
        this.timKiem();
  ...
  updateDonViTinh(id: number, hieuLuc: boolean) {
    hieuLuc 
    id
    ...
    if (this.authService.hasPermission(this.documentTypeDonViTinh, SecurityOperation.Update)) {

      this.dialog.open(ConfirmComponent, {
        disableClose: false,
        width: '400px',
        data: { message: CommonService.format(SystemMessage.MessLockTemplate, [comfirm, "đơn vị tính"]) }
      }).afterClosed().subscribe(result => {/* result is a string:Yes,No,No answer*/
        if (result == true) {
          self.apiService.post("QuanTriNhomDuocPhamDonViTinh/KichHoatHieuLuc?id=" + id).subscribe(
            () => {
              self.table.search();
              if (hieuLuc)
