...
@Component({
  selector: 'app-dich-vu-ky-thuat',
  templateUrl: './dich-vu-ky-thuat.component.html',
  styleUrls: ['./dich-vu-ky-thuat.component.scss']
})
export class DichVuKyThuatComponent implements OnInit {

  id: number = null;
  dataChon: any;
  now:Date=new Date();
  @ViewChild("tableDichVuKyThuat", { static: true }) table:TableComponent;
  ...
  dichVuKyThuatQueryInfo:any={};
  dichVuKyThuatSearching:boolean=false;
  columnsDichVuKyThuat: TableColumn<any>[] = [];
  groupByColumnsDichVuKyThuat: string[] = [];
  sortByColumnDichVuKyThuat: SortDescriptor = { field: 'Ma', dir: 'desc' };
  documentTypeDichVuKyThuat: DocumentType;

  initDichVuKyThuat() {
    this.documentTypeDichVuKyThuat = DocumentType.QuanTriNhomPhongKhamDichVuKyThuat;
    this.columnsDichVuKyThuat = [
  ...
  timKiem() {
    this.table.additionalSearchObject = this.dichVuKyThuatQueryInfo;
    this.table.search();
    this.dangTimKiem=true;
    this.dichVuKyThuatSearching=true;
  }
  huyTimKiem(){
    this.dichVuKyThuatQueryInfo={};
    this.timKiem();
    this.dangTimKiem=false;
    this.dichVuKyThuatSearching=false;
  }
  onKey(event: any) {
    if (event.key == 'Enter') {
        this.timKiem();
  ...
  updateDichVuKyThuat(id: number, hieuLuc: boolean) {
    hieuLuc 
    id
    ...
    if (this.authService.hasPermission(this.documentTypeDichVuKyThuat, SecurityOperation.Update)) {

      this.dialog.open(ConfirmComponent, {
        disableClose: false,
        width: '400px',
        data: { message: CommonService.format(SystemMessage.MessLockTemplate, [comfirm, "dịch vụ kỹ thuật"]) }
      }).afterClosed().subscribe(result => {/* result is a string:Yes,No,No answer*/
        if (result == true) {
          self.apiService.post("QuanTriNhomPhongKhamDichVuKyThuat/KichHoatHieuLuc?id=" + id).subscribe(
            () => {
              self.table.search();
              if (hieuLuc)
