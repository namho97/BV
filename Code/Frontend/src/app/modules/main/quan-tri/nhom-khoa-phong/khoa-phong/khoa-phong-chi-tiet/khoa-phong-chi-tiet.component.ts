...
@Component({
  selector: 'app-khoa-phong-chi-tiet',
  templateUrl: './khoa-phong-chi-tiet.component.html',
  styleUrls: ['./khoa-phong-chi-tiet.component.scss']
})
export class KhoaPhongChiTietComponent implements OnInit {
  ...
  constructor(private dialog: MatDialog, private baseService: BaseService, private snackBar: MatSnackBar, private errorService: ErrorService,
    private khoaPhongService: KhoaPhongService, private authService: AuthService,
    private cdRef: ChangeDetectorRef) {
    this.baseService.controllerName = this.khoaPhongService.controllerName;
  }
  ...
  capNhat() {
    ...
    if (this.authService.hasPermission(this.documentType, SecurityOperation.Update)) {
      let dialogRef = this.dialog.open(ConfirmComponent, {
        data: {
          message: "Bạn chắc chắn muốn cập nhật khoa phòng này?"
        }
      });
  ...
  luu() {
    ...
    if (this.authService.hasPermission(this.documentType, SecurityOperation.Add)) {
      let dialogRef = this.dialog.open(ConfirmComponent, {
        data: {
          message: "Bạn chắc chắn muốn lưu khoa phòng này?"
        }
      });
      dialogRef.afterClosed().subscribe(result => {
        if (result == true) {
  ...
  hieuLucChange(event:any){
    if(event != null){
      if(event == 1){
        this.khoaPhongVo.HieuLuc = true;
      }
      if(event == 2)
      {
        this.khoaPhongVo.HieuLuc = false;
      }
    }
    else{
      this.khoaPhongVo.HieuLuc = null;
    }
