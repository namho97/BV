...
@Component({
  selector: 'app-khoa-phong-nhan-vien-chi-tiet',
  templateUrl: './khoa-phong-nhan-vien-chi-tiet.component.html',
  styleUrls: ['./khoa-phong-nhan-vien-chi-tiet.component.scss']
})
export class KhoaPhongNhanVienChiTietComponent implements OnInit {

  id :number = null;
  khoaPhongNhanVienVo: any = {};
  ...
  constructor(private dialog: MatDialog, private baseService: BaseService, private snackBar: MatSnackBar, private errorService: ErrorService,
    private KhoaPhongNhanVienService: KhoaPhongNhanVienService, private authService: AuthService,
    private cdRef: ChangeDetectorRef) {
    this.baseService.controllerName = this.KhoaPhongNhanVienService.controllerName;
  }
  ...
  getById(id) {
    if (id != null && id > 0) {
      this.baseService.getById(id).subscribe((result) => {
        this.khoaPhongNhanVienVo = result;
      });
    }
  }
  quayLai() {
  ...
  capNhat() {
    ...
    if (this.authService.hasPermission(this.documentType, SecurityOperation.Update)) {
      let dialogRef = this.dialog.open(ConfirmComponent, {
        data: {
          message: "Bạn chắc chắn muốn cập nhật khoa phòng - nhân viên này?"
        }
      });
      dialogRef.afterClosed().subscribe(result => {
        if (result == true) {
          this.showPopupLoadingData();
          this.baseService.update(this.khoaPhongNhanVienVo).subscribe(() => {
            this.dataChange.emit(this.khoaPhongNhanVienVo);
  ...
  luu() {
    ...
    if (this.authService.hasPermission(this.documentType, SecurityOperation.Add)) {
      let dialogRef = this.dialog.open(ConfirmComponent, {
        data: {
          message: "Bạn chắc chắn muốn lưu khoa phòng -nhân viên này?"
        }
      });
      dialogRef.afterClosed().subscribe(result => {
        if (result == true) {
          this.showPopupLoadingData();
          this.baseService.create(this.khoaPhongNhanVienVo).subscribe((result) => {
            this.khoaPhongNhanVienVo = {};
