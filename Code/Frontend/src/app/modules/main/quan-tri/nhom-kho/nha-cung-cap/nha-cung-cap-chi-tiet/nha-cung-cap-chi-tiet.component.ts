...
@Component({
  selector: 'app-nha-cung-cap-chi-tiet',
  templateUrl: './nha-cung-cap-chi-tiet.component.html',
  styleUrls: ['./nha-cung-cap-chi-tiet.component.scss']
})
export class NhaCungCapChiTietComponent implements OnInit {

  id:number = null;
  nhaCungCapVo: any = {};
  validationErrors: any = null;
  documentType: DocumentType = DocumentType.QuanTriNhomKhoNhaCungCap;
  ...
  constructor(private dialog: MatDialog, private baseService: BaseService, private snackBar: MatSnackBar, private errorService: ErrorService,
    private nhaCungCapService: NhaCungCapService, private authService: AuthService,
    private cdRef: ChangeDetectorRef) {
    this.baseService.controllerName = this.nhaCungCapService.controllerName;
  }
  ...
  capNhat() {
    ...
    if (this.authService.hasPermission(this.documentType, SecurityOperation.Update)) {
      let dialogRef = this.dialog.open(ConfirmComponent, {
        data: {
          message: "Bạn chắc chắn muốn cập nhật nhà cung cấp này?"
        }
      });
      dialogRef.afterClosed().subscribe(result => {
        if (result == true) {
          this.showPopupLoadingData();
          this.baseService.update(this.nhaCungCapVo).subscribe(() => {
            this.dataChange.emit(this.nhaCungCapVo);
  ...
  luu() {
    ...
    if (this.authService.hasPermission(this.documentType, SecurityOperation.Add)) {
      let dialogRef = this.dialog.open(ConfirmComponent, {
        data: {
          message: "Bạn chắc chắn muốn lưu nhà cung cấp này?"
        }
      });
      dialogRef.afterClosed().subscribe(result => {
        if (result == true) {
          this.showPopupLoadingData();
          this.baseService.create(this.nhaCungCapVo).subscribe((result) => {
            this.nhaCungCapVo = {};
