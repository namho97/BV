...
export class InBaoCaoComponent implements OnInit {
  ...
  getContentInBaoCao(){
    ...
    var Template="<table width='100%' class='table-dich-vu'><tr><th>STT</th><th>MÃ NB</th><th>HỌ TÊN</th><th>GIỚI TÍNH</th><th>NGÀY SINH</th><th>ĐIỆN THOẠI</th><th>ĐỊA CHỈ</th><th>NGÀY KHÁM</th><th>TRẠNG THÁI</th><th>LÝ DO KHÁM BỆNH</th><th>TỔNG TRẠNG</th><th>ICD</th><th>NỘI DUNG CẨN ĐOÁN</th><th>CÁCH GIẢI QUYẾT</th><th>LỜI DẶN</th><th>NGÁY TÁI KHÁM</th><th>GHI CHÚ</th></tr>";
    var i=1;
    this.data.Data.forEach(item => {
      ...
      Template+="<td>"+item.NgayKhamBenhHienThi+"</td>";
      Template+="<td>"+item.TrangThaiHienThi+"</td>";
      Template+="<td>"+(item.LyDoKhamBenh??'')+"</td>";
      Template+="<td>"+(item.TongTrang??'')+"</td>";
      Template+="<td>"+(item.TenIcd??'')+"</td>";
      Template+="<td>"+(item.NoiDungChanDoan??'')+"</td>";
      Template+="<td>"+(item.CachGiaiQuyetHienThi??'')+"</td>";
      Template+="<td>"+(item.LoiDan??'')+"</td>";
      Template+="<td>"+(item.NgayTaiKhamHienThi??'')+"</td>";
      Template+="<td>"+(item.GhiChu??'')+"</td>";
      Template+="</tr>";
      i++;
    });
    Template+="</table>";
